...
export function getIsAfterDecree(
  yearOfIndexation: number,
  region: Regions,
  agreementStartDate: Date,
) {
  return (
    yearOfIndexation > ENERGY_RATIOS[region].end.getFullYear() ||
    (yearOfIndexation === ENERGY_RATIOS[region].end.getFullYear() &&
      agreementStartDate.getMonth() > ENERGY_RATIOS[region].end.getMonth())
  );
}
...
export function getInitialIndex(
  ...
  if (agreementStartDate < new Date('1994-02-01')) {
    ...
    return roundToTwoDecimals(
      findHealthIndex(agreementSignatureYear, agreementSignatureMonth, selectedIndexBaseYear),
    );
  }
  ...
  const initialHealthIndex = findHealthIndex(
    initialIndexYear,
    initialIndexMonth,
    selectedIndexBaseYear,
  );

  return roundToTwoDecimals(initialHealthIndex);
}
...
export function findHealthIndex(year: number, month: string, baseYear: number) {
  const indexEntry = healthIndexData.facts.find((entry) => {
    return (
      ...
      entry['Month'] === month &&
      entry['Base year'] === baseYear.toString()
    );
  });

  if (!indexEntry)
    throw new Error(
      'Error: Could not find health index for : date :' +
        year +
        ' ' +
        month +
        ' - base year :' +
...
export const getAnniversaryMonth = (agreementStartDate: Date) =>
  subMonths(agreementStartDate, 1).toLocaleString('en-US', { month: 'long' });
...
export const deriveData = ({
  ...
  const newHealthIndex = roundToTwoDecimals(
    findHealthIndex(
      shouldUsePreviousYear(anniversaryMonth) ? yearOfIndexation - 1 : yearOfIndexation,
      anniversaryMonth,
      indexBaseYear,
    ),
  );
  const isRequestedAfterEndOfDecree = getIsAfterDecree(
    yearOfIndexation,
    region,
    agreementStartDate,
  );

  const wasIndexationRequestedBeforeStartOfEnergyRatingDecree =
    yearOfIndexation <= ENERGY_RATIOS[region].start.getFullYear() &&
    agreementStartDate.getMonth() < ENERGY_RATIOS[region].start.getMonth();

  const healthIndexBeforeDecree = roundToTwoDecimals(
    findHealthIndex(
      shouldUsePreviousYear(anniversaryMonth) ? yearOfIndexation - 2 : yearOfIndexation - 1,
      anniversaryMonth,
      indexBaseYear,
    ),
  );
...
export const deriveDataWithPEB = ({
  ...
  basicFormulaWithInitialRentAndIndex: (index: number) => number;
}) => {
  const yearOfIndexationWithPEB = getYearOfIndexationWithPEB(agreementStartDate, region);

  const currentYearHealthIndex = findHealthIndex(
    shouldUsePreviousYear(anniversaryMonth) ? yearOfIndexationWithPEB - 1 : yearOfIndexationWithPEB,
    anniversaryMonth,
    indexBaseYear,
  );

  const previousYearHealthIndex = findHealthIndex(
    shouldUsePreviousYear(anniversaryMonth)
      ? yearOfIndexationWithPEB - 2
      : yearOfIndexationWithPEB - 1,
    anniversaryMonth,
    indexBaseYear,
  );
